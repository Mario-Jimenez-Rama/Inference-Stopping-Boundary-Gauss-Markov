...
compute_proportions_from_files <- function(M = 100, delta) {
  ...
  for (trial_index in 1:M) {
    x_sample_file <- file.path(folder_name, paste0("path_OU_", trial_index, ".rds"))
    inferred_boundary_file <- file.path(folder_name, paste0("inferred_boundary_OU_", trial_index, ".rds"))
    
    # Load the data
    X_sample <- readRDS(x_sample_file)
    inferred_results <- readRDS(inferred_boundary_file)
    if (trial_index==100){
      cat("inferred_boundary =", inferred_results$boundary_est, "\n")
      cat("inferred_boundary lower =", inferred_results$lower_bound, "\n")
      cat("inferred_boundary upper =", inferred_results$upper_bound, "\n")
    }
